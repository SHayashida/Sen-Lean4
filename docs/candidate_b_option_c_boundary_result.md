# Candidate B: Option C Voter-Boundary Result (M2.1)

**M2.1 status:** See [docs/m2_1_status_summary.md](m2_1_status_summary.md) for the current two-sided summary.

## 1. Purpose

This note records the M2.1 Option C / `(3,4)` Step 0 voter-dimension boundary
result. Option C is an exploratory boundary comparison; it is not the Option D
positive track and does not authorize repair enumeration.

## 2. Relation to Option D

Option D is the completed positive track. It shows that M1.5
repair-presentation non-canonicity persists at `(2,5)` under clause-multiset
equivalence (≡CM) while the voter count remains two and the alternative
dimension expands.

Option C asks the separate voter-dimension question: what happens when the
split representation must select two witnesses among three voters? It does not
re-open or weaken the completed Option D positive result.

## 3. Option C Setup

The bundled package is the repository's existing `(3,4)` parametric `minlib`
encoding using `pair_selectors_v1`.

The exploratory split package uses the transport map:

```text
π(minlib)    = {selected_decisive_left, selected_decisive_right}
π(asymm)     = {asymm}
π(un)        = {un}
π(no_cycle3) = {no_cycle3}
π(no_cycle4) = {no_cycle4}
```

`π` is a lever correspondence and repair-transport map, not a semantic
equivalence claim between axiom formulas.

Both split levers share one witness-pair selector bank. They use separate
left/right decisive-selector banks so that each lever exposes its witness role
independently.

## 4. Step 0 Result

The target case is `(n,m) = (3,4)`:

| Measurement | Bundled | Split |
| --- | ---: | ---: |
| SAT status | `UNSAT` | `UNSAT` |
| Variables | 165,927 | 165,939 |
| Clauses | 1,347,847 | 1,513,735 |

The split side therefore adds 12 variables and 165,888 clauses. SAT/UNSAT status
agrees, clause-multiset equivalence is `false`, and the Step 0 classification is
`sat_equiv_only`.

## 5. Cause of ≡CM Failure

Status agrees, so the impossibility status survives this encoding comparison.
However, the split package introduces unmatched role-specific
decisive-selector structure.

At `(3,4)`, voter 1 can be the right witness in pair `(0,1)` and the left
witness in pair `(1,2)`. The split representation therefore gives voter 1
distinct left/right role-specific decisive-selector series. These selectors and
their 165,888 additional binary implication clauses have no one-to-one
counterpart in bundled `minlib`.

The distinct-variable count, clause count, and clause-length histogram all
differ. These are invariants under variable renaming, so no bijection `ρ` can
make the two clause multisets equal. Neither identity nor renamed
clause-multiset equivalence can therefore be established.

## 6. Interpretation

Status transfers at `(3,4)`, but the M1.5-style ≡CM shield does not. Therefore
the strong M1.5-style, ≡CM-protected repair-representation non-canonicity claim
is not authorized for Option C at `(3,4)`.

Because ≡CM fails, any later Option C repair divergence would be weaker than the
Option D result: selector or encoding-strength explanations would not have been
ruled out.

This is a voter-dimension representation boundary, not an Option D regression.
It is not a failure of M2.1. The evidence supports the narrower statement that
the M1.5 witness class transfers cleanly along the two-voter
alternative-expansion track but does not automatically transfer along the
voter-dimension track.

## 7. Gate Consequence

Option C repair enumeration and MCS enumeration remain unauthorized after the
`sat_equiv_only` classification. Do not run Option C repair enumeration unless a
later explicit decision accepts the weaker `sat_equiv_only` framing.

The current strong M2.1 repair-representation claim relies on Option D, not
Option C. This Step 0 result must not be used as a basis for an ≡CM-protected
repair claim.

## 8. Two-Sided M2.1 Shape

The current M2.1 result is two-sided:

- **Positive side — Option D / `(2,5)`:** `equiv_cm_persists` and
  `noncanonical_persists`.
- **Boundary side — Option C / `(3,4)`:** `sat_equiv_only`; status agrees, but
  selector machinery breaks ≡CM.

This mirrors the broader program's discipline of recording both what transfers
and where transfer stops.

## 9. Artifact Pointers

- [`scripts/exploration/candidate_b/option_c_encoder.py`](../scripts/exploration/candidate_b/option_c_encoder.py)
- [`scripts/exploration/candidate_b/step0_equiv_check_option_c.py`](../scripts/exploration/candidate_b/step0_equiv_check_option_c.py)
- `/tmp/candidate_b_step0_option_c/step0_option_c_comparison.json`

The `/tmp` report is a local, regenerable output and is not a tracked
repository artifact.

## 10. Scope Caveat

This result is local-rationality only:
`no_cycle3 ∧ no_cycle4` is not full `SocialAcyclic` at family scale.

M2 is unchanged, and the theorem `sen_impossibility_lifts` is unchanged. No M2
theorem or paper claim is modified. The sen24 CNF is not a family-level CNF
certificate.
