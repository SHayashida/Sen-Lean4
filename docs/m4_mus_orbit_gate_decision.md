...
This is a STRONG GO / WEAK GO / CONDITIONAL GO / NO-GO gate for directing M4
work. It is not an M4 theorem, not a Lean formalization, and not a change to
the dissertation scope.
...
O2/O3/O4 shape dependence, then a separate decision document may consider
whether M4 scope should be changed to incorporate semantic MUS orbit structure
before warrant-contract implementation.
...
only a design for a three-voter semantic `S_3` extension precheck.
