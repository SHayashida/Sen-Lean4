...
name = "decisive_voter0"
category_key = "decisive_voter0"
...
def encode(schema: "FiniteSchema", out_clauses: list[list[int]]) -> None:
    if schema.minlib_mode != "selectors_v1":
        raise ValueError(
            "decisive_voter0 requires legacy selectors_v1 mode and is defined only for the Sen24 base case"
        )

    out_clauses.append([schema.var_sel0(a, b) for (a, b) in schema.pairs])
    for p in range(schema.n_profiles):
        for a, b in schema.pairs:
            lit = schema.var_p(p, a, b) if schema.pref0(p, a, b) else schema.var_p(p, b, a)
            out_clauses.append([-schema.var_sel0(a, b), lit])
