...
M2 separates two layers that the surrounding program treats as independent.
...
M2's main positive deliverable belongs to the Proof layer; M2's main negative deliverable concerns the wall between the two layers.
...
(iii) the Negative \#1 scope wall, and
(iv) the artifact binding for both deliverables.
The paper does \emph{not} contain
(i) new solver experiments,
(ii) any modification of the M1 or M1.5 manuscripts,
(iii) a family-level CNF certificate for full acyclicity, or
(iv) a Candidate-B / Negative \#2 family-scale representation-canonicity result.
These last two are explicitly deferred to M2.1.

\paragraph{Public-repo safety.}
The paper references repository-relative paths only.
It does not contain local absolute paths, secrets, or internal-only infrastructure details, in line with \code{AGENTS.md} \S0.2 and \code{docs/public\_repo\_security.md}.
The protected baseline artifacts \code{Certificates/sen24.cnf}, \code{Certificates/sen24.manifest.json}, \code{Certificates/sen24.lrat}, and \code{Certificates/atlas/case\_11111/**} are untouched by this paper, in line with \code{AGENTS.md} \S9.1.
