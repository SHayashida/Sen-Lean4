...
The generic bridge theorem ranges over an arbitrary voter type and a finite decidable alternative type under the repository's ranking interface.
The legacy theorem \(\mathtt{sen\_impossibility\_lifts}\) remains available for all finite cases \((\mathtt{Fin}\ n, \mathtt{Fin}\ m)\) with \(2 \leq n\) and \(4 \leq m\), as a compatibility corollary.
This does not claim a new result for arbitrary social-choice models outside the repository definitions; it does not claim a result for Arrow's theorem; and it does not claim a result that bypasses \(\mathtt{UN}\) or \(\mathtt{MINLIB}\).
The phrase ``finite semantic obstruction bridge'' in this paper always means the semantic Proof-layer theorem in the sense above.
...
The negative-side scope wall is exactly the statement that the Proof-layer bridge does not by itself extend this audited equivalence.

\paragraph{What is in this paper, what is not.}
The paper contains
(i) the polymorphic Lean signatures used by the obstruction bridge,
(ii) the kernel-checked theorem ladder from witness extraction to \(\neg\,\mathtt{SocialAcyclic}\),
(iii) the Proof-vs-Witness/Audit scope wall, and
(iv) the artifact binding for the bridge, compatibility corollary, and scope wall.
