...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \draw[<->, thick, blue!65!black] (implres) -- node[above, font=\scriptsize,
      fill=white, inner sep=1.5pt] {\(\ResidualFaithfulness\)} (refres);
    \draw[arrow, green!55!black] (raw) -- node[above, font=\scriptsize,
      fill=white, inner sep=1.5pt] {touch-any grouping} (group);
    \draw[arrow, green!55!black] (group) -- node[right, font=\scriptsize,
...
In the displayed notation \(\forall T'\subseteq T\subseteq I\), the intended
meaning is \(T'\subseteq T\) and \(T\subseteq I\), so \(T'\subseteq I\) follows
by transitivity.  This matches the Lean definition shown in
Appendix~\ref{app:lean}.  The monotonicity direction is from larger retained
sets to smaller retained sets: deleting more contract atoms cannot destroy
