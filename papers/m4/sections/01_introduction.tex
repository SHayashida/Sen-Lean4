...
relevant finite evidence to be audited completely, but rich enough to expose
the gap between an impossibility result and a justified repair report. It
contains multiple requirements, multiple possible repairs, and nontrivial ways
to group those repairs for explanation. M4 uses this case because it is a
controlled setting in which the evidence boundary can be made explicit. It
...
This paper is deliberately not an end-to-end verification result. M4 does not
prove Python encoder correctness, CNF artifact correctness, full
semantic-to-CNF correctness, full selector-free equivalence to Lean
\code{MINLIB}, a new general Sen theorem, or family-scale CNF/LRAT/atlas/repair
lifts. The general Sen theorem belongs to the M2 semantic bridge and is not a
new M4 claim. The M4 claim is narrower and more explicit: under the declared
finite encoding, the paper audits which repair and reporting statements are
licensed, while marking the remaining artifact-correctness bridges as future
...
contracts. Section~8 records limitations and future correctness obligations.
