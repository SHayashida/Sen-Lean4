# Atlas Summary

## Headline

- atlas_schema_version: `1`
- cases_total: `16`
- SAT: `15`
- UNSAT: `1`
- UNKNOWN: `0`

## Runtime

- solver_path: `/opt/homebrew/Cellar/cadical/3.0.0/bin/cadical`
- solver_version_raw: `3.0.0`
- solver_version: `3.0.0`
- solver_sha256: `eee5786b12f606ed94339c128156e568b523fa12a2e3870774d19a1b3725988e`
- python_version: `3.9.6`
- platform: `macOS-15.6.1-arm64-arm-64bit`
- git_commit: `98a21b660958abb40e80f49a5a29ca75c7e6eb04`

## Symmetry classes

- mode: `none`
- symmetry_unsafe_axioms: `[]`
- equivalence classes: `16`
- UNSAT classes: `1`
- symmetry_check: enabled=`False` checked_k=`0` mismatches=`0`
- checked_cases: `[]`
- representatives (16): `case_00000`, `case_00001`, `case_00100`, `case_00101`, `case_01000`, `case_01001`, `case_01100`, `case_01101`, `case_10000`, `case_10001`, `case_10100`, `case_10101`, `case_11000`, `case_11001`, `case_11100`, `case_11101`

## Pruning

- mode: `none`
- solver_calls: `16`
- solver_calls_avoided: `0`
- pruned_sat: `0`
- pruned_unsat: `0`
- oracle_sat_inferred: `0`
- oracle_unsat_inferred: `0`

## UNSAT cases

- `case_11101` solved=`True` on=['asymm', 'un', 'minlib', 'no_cycle4']
  - mus(size=4): ['asymm', 'un', 'minlib', 'no_cycle4']
  - mcs(remove=1): ['asymm']

## Closest SAT cases

- `case_11100` on_size=3 solved=`True` on=['asymm', 'un', 'minlib']
- `case_11001` on_size=3 solved=`True` on=['asymm', 'un', 'no_cycle4']
- `case_10101` on_size=3 solved=`True` on=['asymm', 'minlib', 'no_cycle4']
- `case_01101` on_size=3 solved=`True` on=['un', 'minlib', 'no_cycle4']
- `case_11000` on_size=2 solved=`True` on=['asymm', 'un']

## Class frontier

- `eq_28800e6a76f6f425` rep=`case_11101` orbit=1 status={'UNSAT': 1}

## Frontier note

UNSAT case -> one-step SAT repair candidate via MCS:
- `case_11101` --remove ['asymm']--> `case_01101` (`sat_mask=22`)
